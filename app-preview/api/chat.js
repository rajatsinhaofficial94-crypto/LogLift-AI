export default async function handler(req, res) {
  ...
  try {
    const { message, context, history = [], selectedModel = 'anthropic' } = req.body;
    ...
    if (selectedModel === 'groq' && !groqKey) {
      return res.status(500).json({ error: 'GROQ_API_KEY is missing' });
    }
    if (selectedModel !== 'groq' && !anthropicKey) {
    ...
    if (selectedModel === 'groq') {
      ...
      if (!groqRes.ok) {
        ...
        return res.status(groqRes.status).json({
          ...
          errorType: errObj.error?.type || 'groq_error',
        });
      }

      const groqData = await groqRes.json();
      reply = groqData.choices?.[0]?.message?.content || "I'm sorry, I couldn't generate a response.";
    } else {
      // Anthropic with retry on overload
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        return res.status(response.status).json({
          ...
          errorType: errorObj.error?.type || errorObj.type || 'unknown',
        });
      }
    ...
    res.status(200).json({ reply, pineconeStatus, modelUsed: selectedModel });
  } catch (error) {
